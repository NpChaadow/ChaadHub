...
function CreateTabFrame(x,y,xsize,ysize, Name)
	...
	local HubLabel = CreateTextLabel(0.4,0.070,0.5,0.16,"HubNameLabel","Chaad Hub v.0.2.4c",Color3.new(0.458824, 0.458824, 0.458824),Color3.new(1, 1, 1))
...
local TowerTeleportButton = CreateTextButton(0.2,.45,.4,0.125,"TowerTeleportButton", "Teleport To Tower",Color3.new(0.294118, 0, 0.00392157),Color3.new(1, 1, 1))
	...
	while AutoBuy do
		...
		for i,v in pairs(PlayerTycoon.ButtonFolder:GetChildren())do
			...
			if CrateFound ~= true and AutoBuy then
				...
				if v:FindFirstChild("Button") ~= nil and v.Button.Color.R < v.Button.Color.G and v.Button.Color.B < ((v.Button.Color.R + v.Button.Color.G)/2) then		
					Fnd = true
...
function TeleportTower()
	...
	GoToPoint(character.PrimaryPart.Position, Vector3.new(-7, 800, -433),40,0.1)
